...
class GuzarDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('ناحیه', fn($row) => $row->district->district_Name ?? 'ناموجود')
            ->addColumn('گذر', fn($row) => $row->guzer_Number ?? 'ناموجود')
            ->addColumn('عمل', function ($row) {
                ...
                if (auth()->user()->can('delete_guzar')) {
                    $deleteBtn = '
                        <form action="' . route('guzar.delete', [$row->id]) . '" method="POST" style="display:inline;" id="delete-form-' . $row->id . '">
     ...
     */
    public function query(guzar $model): QueryBuilder
    {
        $query = $model->with(['district'])->orderByDesc('id');
