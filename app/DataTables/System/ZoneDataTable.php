...
class ZoneDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('زون', fn($row) => $row->zone_Name ?? 'ناموجود')
            ->addColumn('عمل', function ($row) {
                ...
                if (auth()->user()->can('delete_zone')) {
                    $deleteBtn = '
                        <form action="' . route('zone.delete', [$row->id]) . '" method="POST" style="display:inline;" id="delete-form-' . $row->id . '">
                ...
                if (auth()->user()->can('edit_zone')) {
                    $editBtn = '
                                <button
                                    class="btn btn-success sharp edit-zone"
                                    data-id="' . $row->id . '"
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('زون')->addClass('text-center'),
            Column::computed('عمل')
                ->exportable(false)
                ->printable(false)
                ->width(200)
